...
const projects = [
  {
    ...
    admin: 'https://coreflowpersonal-production.up.railway.app/admin/',
...
- Thymeleaf 기반 화면에서 서버 데이터와 바인딩되며, 카카오 로그인/카카오페이, ChatGPT 챗봇 등 외부 API와 연동됩니다.
- 서비스는 Railway 환경에 배포되어 실제 운영 환경에서 동작합니다.`,
      features: [
        '카카오 소셜 로그인 - OAuth2.0 기반 카카오 계정 간편 로그인 구현',
        '카카오페이 결제 - 결제 준비/승인/취소 API 연동 및 주문 상태 관리',
        'ChatGPT AI 챗봇 - ChatGPT API 연동으로 FAQ 자동 응답 구현',
      ...
      trouble: [
        { problem: '카카오페이 결제 후 중복 주문 생성', solution: '결제 완료 후 브라우저 뒤로가기 시 주문이 중복 생성되는 문제를 PRG 패턴(Post-Redirect-Get) 적용으로 해결' },
        { problem: '이미지 업로드 경로 문제', solution: '로컬 개발 환경의 절대경로가 배포 서버에서 동작하지 않아 환경변수로 업로드 경로를 분리하여 로컬/서버 환경 각각 적용' },
        { problem: '카카오 로그인 후 세션 충돌', solution: '카카오 로그인 시 기존 일반 로그인 세션과 충돌이 발생하여 세션 초기화 후 새 세션 발급으로 해결' },
        { problem: 'gitignore로 인한 배포 환경변수 문제', solution: '보안을 위해 application-private.properties를 gitignore에 추가했으나 Railway가 파일을 읽지 못해 Railway Variables에 환경변수를 직접 등록하여 해결' },
      ]
    },
  },
  {
...
- 서비스는 Railway 환경에 배포되어 실제 운영 환경에서 동작합니다.`,
      ...
      trouble: [
        { problem: 'Redis 인증 오류', solution: 'Railway Redis는 비밀번호 인증이 필요한데 설정이 없어 연결 실패. REDIS_PASSWORD 환경변수를 Railway Variables에 등록하여 해결' },
        { problem: 'WebSocket Spring Security 충돌', solution: 'Spring Security가 WebSocket 엔드포인트를 막아 연결 실패. SecurityConfig에서 WebSocket 경로와 STOMP 엔드포인트를 permitAll로 설정하여 해결' },
        { problem: 'ChatMessageDto isRead null 역직렬화 오류', solution: 'primitive boolean 타입에 null이 들어와 역직렬화 실패. Boolean 래퍼 타입으로 변경하여 해결' },
        { problem: 'Redis 변수명 점(.) 포함으로 빌드 실패', solution: 'Railway BuildKit이 점(.)이 포함된 환경변수를 secret으로 인식하여 빌드 실패. 변수명을 언더바(_)로 변경하여 해결' },
      ]
    },
  },
  {
...
- 서비스는 Cloudtype 환경에 MariaDB와 함께 배포되어 동작합니다.`,
      ...
      trouble: [
        { problem: '일정 삭제 시 외래키 제약 오류', solution: '공유된 일정 삭제 시 schedule_shares 테이블의 외래키 제약으로 삭제 실패. 일정 삭제 전 연관된 공유 데이터를 먼저 삭제하도록 순서 변경' },
        { problem: '로컬/배포 DB 환경 차이', solution: '로컬은 MySQL, 배포는 MariaDB를 사용하여 드라이버와 Dialect가 달라 충돌 발생. application-local.properties와 application-prod.properties로 프로파일 분리하여 해결' },
        { problem: 'FullCalendar 종료일 하루 전까지만 표시', solution: 'FullCalendar의 end 날짜는 exclusive라 실제 종료일이 표시되지 않음. endDate에 plusDays(1)을 적용하여 종료일까지 정확하게 표시' },
      ]
    },
  },
]
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              ...
              {selected.admin && (
                <a href={selected.admin} target="_blank" rel="noreferrer" style={{
